...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './paymentrequestapproval.component.html'
})

export class PaymentRequestApprovalComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  previewFile(row1, row2) {
    ...
    this.dalservice.PriviewFile(usersJson, this.APIControllerInvoice, this.APIRouteForPriviewFile)
      .subscribe(
        (res) => {
          const parse = JSON.parse(res);
  ...
  //(+) Ari 2024-03-28
  //#region button edit
  btnEdits(codeEdit: string) {
    this.route.navigate(['objectinfopaymentrequest/subinvoiceregister', codeEdit, this.param]);
  }
  //#endregion button edit
  //(+) Ari 2024-03-28
