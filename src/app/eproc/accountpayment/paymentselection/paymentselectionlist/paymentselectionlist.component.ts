...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './paymentselectionlist.component.html'
})

export class PaymentSelectionlistComponent extends BaseComponent implements OnInit {
  // variable
  ...
  private RoleAccessCode = 'R00021620000000A'; // role access
  ...
  loadData() {
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIControllerInvoice, this.APIRouteForPaymentSelectionGetRows).subscribe(resp => {
  ...
  //#region button add
  // btnAdd() {
  //   this.route.navigate(['/accountpayable/subpaymentselectionlist/invoiceregisterdetail']);
  // }
  //#endregion button add

  //#region button edit
  // btnEdit(codeEdit: string) {
  //   this.route.navigate(['/accountpayable/subpaymentselectionlist/invoiceregisterdetail', codeEdit]);
  // }
        ...
        if (result.value) {
          ...
          (function loopProcurementProceed() {
            if (i < th.checkedList.length) {
              ...
              th.dalservice.ExecSp(th.dataTampProceed, th.APIControllerInvoice, th.APIRoutePaymentSelectionForProcess)
                .subscribe(
                  res => {
                    const parse = JSON.parse(res);
                    if (parse.result === 1) {
  ...
  selectAllTable() {
    for (let i = 0; i < this.listPaymentRequest.length; i++) {
      if (this.listPaymentRequest[i].is_calculated !== '1') {
