...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './quotationreviewdocumentwizdetail.component.html'
})

export class QuotationReviewDocumentWizDetailComponent extends BaseComponent implements OnInit {
  // get param from url
            ...
            if (parse.result === 1) {
              if (this.tamps.length != 0) {
                this.parameterData.p_file.push({
                  p_header: 'PROCUREMENT_QUATATION_DOCUMENT',
                  p_child: this.param,
                  p_id: parse.id,
