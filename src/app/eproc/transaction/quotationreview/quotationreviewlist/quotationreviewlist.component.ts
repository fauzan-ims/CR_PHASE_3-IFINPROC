...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './quotationreviewlist.component.html'
})

export class QuotationReviewlistComponent extends BaseComponent implements OnInit {
  // variable
  public listprocquoreview: any = [];
  ...
  private RoleAccessCode = 'R00021550000000A'; // role access 
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          this.listprocquoreview = parse.data;
          if (parse.data != null) {
            this.listprocquoreview.numberIndex = dtParameters.start;
          }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/transaction/subquotationreviewlist/quotationreviewdetail']);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/transaction/subquotationreviewlist/quotationreviewdetail', codeEdit]);
  }
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listprocquoreview.length; i++) {
      if (this.listprocquoreview[i].selected) {
        this.checkedList.push(this.listprocquoreview[i].code);
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listprocquoreview.length; i++) {
      this.listprocquoreview[i].selected = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listprocquoreview.every(function (item: any) {
