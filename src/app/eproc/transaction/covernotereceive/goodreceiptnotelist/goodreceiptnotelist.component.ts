...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './goodreceiptnotelist.component.html'
})

export class CoverNoteReceivelistComponent extends BaseComponent implements OnInit {
  // variable
  public listReceiptNote: any = [];
  ...
  private RoleAccessCode = 'R00024430000001A'; // role access 
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        dtParameters.paramTamp.push({
          'p_cover_note_status': this.tampStatusType,
        })
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          // end checkall
          this.listReceiptNote = parse.data;

          if (parse.data != null) {
            this.listReceiptNote.numberIndex = dtParameters.start;
          }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/transaction/subdocumentreceivelist/covernotedetail']);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/transaction/subdocumentreceivelist/covernotedetail', codeEdit]);
  }
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listReceiptNote.length; i++) {
      if (this.listReceiptNote[i].selected) {
        this.checkedList.push(this.listReceiptNote[i].code);
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listReceiptNote.length; i++) {
      this.listReceiptNote[i].selected = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listReceiptNote.every(function (item: any) {
