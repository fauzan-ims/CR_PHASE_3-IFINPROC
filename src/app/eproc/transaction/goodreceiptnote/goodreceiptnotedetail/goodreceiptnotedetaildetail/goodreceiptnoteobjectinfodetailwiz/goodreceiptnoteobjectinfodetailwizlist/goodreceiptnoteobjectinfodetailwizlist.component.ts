...
export class GoodReceiptNoteDetailwizdetailComponent extends BaseComponent implements OnInit {
    ...
    private tampDocumentCode_stnk: String = '';
    private tampDocumentCode_stck: String = '';
    private tampDocumentCode_keur: String = '';
    ...
    private APIRouteForupdateObjectInfoVhcl: String = 'UpdateForGrn';
    ...
    btnSaveList() {
        ...
        var getSTNK = $('[name="p_stnk"]')
            .map(function () { return $(this).val(); }).get();

        var getSTNKDate = $('[name="p_stnk_date"]')
            .map(function () { return $(this).val(); }).get();

        var getSTNKExpDate = $('[name="p_stnk_exp_date"]')
            .map(function () { return $(this).val(); }).get();

        var getSTCK = $('[name="p_stck"]')
            .map(function () { return $(this).val(); }).get();

        var getSTCKDate = $('[name="p_stck_date"]')
            .map(function () { return $(this).val(); }).get();

        var getSTCKExpDate = $('[name="p_stck_exp_date"]')
            .map(function () { return $(this).val(); }).get();

        var getKEUR = $('[name="p_keur"]')
            .map(function () { return $(this).val(); }).get();

        var getKEURDate = $('[name="p_keur_date"]')
            .map(function () { return $(this).val(); }).get();

        var getKEURExpDate = $('[name="p_keur_exp_date"]')
            .map(function () { return $(this).val(); }).get();

        while (i < getID.length) {
            while (i < getSTNK.length) {
                while (i < getSTNKDate.length) {
                    while (i < getSTNKExpDate.length) {
                        while (i < getSTCK.length) {
                            while (i < getSTCKDate.length) {
                                while (i < getSTCKExpDate.length) {
                                    while (i < getKEUR.length) {
                                        while (i < getKEURDate.length) {
                                            while (i < getKEURExpDate.length) {
                                                let STNKDate = null
                                                if (getSTNKDate[i] !== "") {
                                                    STNKDate = this.dateFormatList(getSTNKDate[i]);
                                                }
                                                let STNKExpDate = null
                                                if (getSTNKExpDate[i] !== "") {
                                                    STNKExpDate = this.dateFormatList(getSTNKExpDate[i]);
                                                }
                                                let STCKDate = null
                                                if (getSTCKDate[i] !== "") {
                                                    STCKDate = this.dateFormatList(getSTCKDate[i]);
                                                }

                                                let STCKExpDate = null
                                                if (getSTCKExpDate[i] !== "") {
                                                    STCKExpDate = this.dateFormatList(getSTCKExpDate[i]);
                                                }
                                                let KEURDate = null
                                                if (getKEURDate[i] !== "") {
                                                    KEURDate = this.dateFormatList(getKEURDate[i]);
                                                }
                                                let KEURExpDate = null
                                                if (getKEURExpDate[i] !== "") {
                                                    KEURExpDate = this.dateFormatList(getKEURExpDate[i]);
                                                }
                                                ...
                                                this.listdataDetail.push(
                                                    this.JSToNumberFloats({
                                                        ...
                                                        // p_exp_date: expDate
                                                        p_stnk: getSTNK[i],
                                                        p_stnk_date: STNKDate,
                                                        p_stnk_exp_date: STNKExpDate,
                                                        p_stck: getSTCK[i],
                                                        p_stck_date: STCKDate,
                                                        p_stck_exp_date: STCKExpDate,
                                                        p_keur: getKEUR[i],
                                                        p_keur_date: KEURDate,
                                                        p_keur_exp_date: KEURExpDate,
                                                    })
                                                );
                                                i++;
                                            } i++;
        ...
        for (let i = 0; i < this.tamps.length; i++) {
            this.datauploadlist.p_file.push({
                ...
                //(+) Ari 2024-03-22 ket : add new upload file
                p_file_paths_stnk: 'IFIN_PROC' + '\\' + 'PROCUREMENT_DOCUMENT' + '\\' + this.param,
                p_file_name_stnk: this.tamps[i].p_file_paths_stnk + '_' + (new Date()).toISOString().replace(/[^\d]/g, '') + '_' + this.tamps[i].p_file_name_stnk,
                p_file_paths_stck: 'IFIN_PROC' + '\\' + 'PROCUREMENT_DOCUMENT' + '\\' + this.param,
                p_file_name_stck: this.tamps[i].p_file_name_stck,
                p_file_paths_keur: 'IFIN_PROC' + '\\' + 'PROCUREMENT_DOCUMENT' + '\\' + this.param,
                p_file_name_keur: this.tamps[i].p_file_name_keur,
                p_type: this.tamps[i].p_type,
                //(+) Ari 2024-03-22
        ...
        for (let i = 0; i < this.lookupPoObjectInfoVHCL.length; i++) {
            if (this.lookupPoObjectInfoVHCL[i].selectedLookup) {
                this.checkedList.push({
                    'ID': this.lookupPoObjectInfoVHCL[i].id,
                })
            }
        }
                ...
                (function loopPoProceesProceed() {
                    if (i < th.checkedList.length) {
                        th.dataTampProceed = [{
                            ...
                            'p_good_receipt_note_detail_id': th.params,
                            'action': '',
                        }];
                        th.dalservice.ExecSp(th.dataTampProceed, th.APIControllerPO, th.APIRouteForAddObjectInfoVhcl)
                            .subscribe(
                                res => {
                                    const parse = JSON.parse(res);
                                    if (parse.result === 1) {
    ...
    //(+) Ari 2024-03-22 ket : add new upload
    //#region button select image stnk
    onUpload_stnk(event, code: String) {
        const files = event.target.files;
        const file = files[0];
    ...
    //#endregion button select image stnk

    //#region button select image stck
    onUpload_stck(event, code: String) {
        const files = event.target.files;
        const file = files[0];
    ...
    //#endregion button select image stck

    //#region button select image keur
    onUpload_keur(event, code: String) {
        const files = event.target.files;
        const file = files[0];
    ...
    //#endregion button select image keur
    //(+) Ari 2024-03-22
    ...
    handleFile(event) {
        ...
        if (this.tempType === 'STNK') {
            this.tamps.push({
                ...
                p_file_paths_stnk: this.tampDocumentCode,
                p_file_name_stnk: this.tempFile,
                p_file_paths_stck: '',
                p_file_name_stck: '',
                p_file_paths_keur: '',
                p_file_name_keur: '',
            ...
            this.tamps.push({
                ...
                p_file_paths_stnk: '',
                p_file_name_stnk: '',
                p_file_paths_stck: this.tampDocumentCode,
                p_file_name_stck: this.tempFile,
                p_file_paths_keur: '',
                p_file_name_keur: '',
                p_type: 'STCK',
                p_base64: this.base64textString
            ...
            this.tamps.push({
                ...
                p_file_paths_stck: '',
                p_file_name_stck: '',
                p_file_paths_keur: this.tampDocumentCode,
                p_file_name_keur: this.tempFile,
                p_type: 'KEUR',
                p_base64: this.base64textString
            });
        }
    ...
    //(+) Ari 2024-03-22 ket : priview file
    //#region button priview image stnk
    previewFile_stnk(row1, row2) {
        this.showSpinner = true;
        const usersJson: any[] = Array.of();
        ...
        usersJson.push({
            p_file_name: row1,
            p_file_paths: row2,
            p_file_name_stnk: row1,
            p_file_paths_stnk: row2
        });
    ...
    //#endregion button priview image stnk

    //#region button priview image stck
    previewFile_stck(row1, row2) {
        this.showSpinner = true;
        const usersJson: any[] = Array.of();
    ...
    //#endregion button priview image stck

    //#region button priview image keur
    previewFile_keur(row1, row2) {
        this.showSpinner = true;
        const usersJson: any[] = Array.of();
    ...
    //#endregion button priview image keur
    // (+) Ari 2024-03-25
    ...
    // (+) Ari 2024-03-25 ket : add new upload
    //#region button delete image stnk
    deleteImage_stnk(code: String, file_name: any, paths: any) {
        const usersJson: any[] = Array.of();
        usersJson.push({
            'p_id': code,
            'p_file_name': file_name,
            'p_file_paths': paths,
            'p_file_name_stnk': file_name,
            'p_file_paths_stnk': paths,
            'p_type': 'STNK DELETE'
        });
    ...
    //#endregion button delete image stck

    //#region button delete image stnk
    deleteImage_stck(code: String, file_name: any, paths: any) {
        const usersJson: any[] = Array.of();
        usersJson.push({
            'p_id': code,
            'p_file_name': file_name,
            'p_file_paths': paths,
            'p_file_name_stck': file_name,
            'p_file_paths_stck': paths,
            'p_type': 'STCK DELETE'
        });
    ...
    //#region button delete image keur
    deleteImage_keur(code: String, file_name: any, paths: any) {
        const usersJson: any[] = Array.of();
        usersJson.push({
            'p_id': code,
            'p_file_name': file_name,
            'p_file_paths': paths,
            'p_file_name_keur': file_name,
            'p_file_paths_keur': paths,
            'p_type': 'KEUR DELETE'
        });
    ...
    //#endregion button delete image keur
    // (+) Ari 2024-03-25
