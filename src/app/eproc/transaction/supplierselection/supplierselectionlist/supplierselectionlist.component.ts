...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './supplierselectionlist.component.html'
})

export class SupplierSelectionlistComponent extends BaseComponent implements OnInit {
  // variable
  public listsuppliercollection: any = [];
  ...
  private RoleAccessCode = 'R00021570000000A'; // role access 
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          this.listsuppliercollection = parse.data;

          if (parse.data != null) {
            this.listsuppliercollection.numberIndex = dtParameters.start;
          }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/transaction/subsupplierselectionlist/supplierselectiondetail']);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/transaction/subsupplierselectionlist/supplierselectiondetail', codeEdit]);
  }
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listsuppliercollection.length; i++) {
      if (this.listsuppliercollection[i].selected) {
        this.checkedList.push(this.listsuppliercollection[i].code);
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listsuppliercollection.length; i++) {
      this.listsuppliercollection[i].selected = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listsuppliercollection.every(function (item: any) {
