...
@Component({
    moduleId: module.id,
    selector: 'app-root',
    templateUrl: './supplierselectiondocumentwizlist.component.html'
})

export class SupplierSelectionDocumentWizListComponent extends BaseComponent implements OnInit {
    // get param from url
    ...
    public tamp_reff_no: Boolean = false;

    private APIController: String = 'SupplierSelectionDocument';
        ...
        this.dtOptions = {
            ...
            ajax: (dtParameters: any, callback) => {
                ...
                this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
                    ...
                    if (parse.data[0] !== undefined) {
                        if (parse.data[0].reff_no !== '' || parse.data[0] === undefined) {
                            this.tamp_reff_no = true;
                        } else {
                            this.tamp_reff_no = false;
                        }
                    }
            ...
            .subscribe(
                res => {
                    ...
                    if (parse.result === 1) {
                        ...
                        // }
                        //   else {
                        //   this.dalservice.UploadFile(this.dataphotolist.p_file, this.APIController, this.APIRouteForUploadFile)
                        //     .subscribe(
                        //       res => {
                        //         const parse = JSON.parse(res);
    ...
    //   this.tamps.push({
    //     p_header: 'PUBLIC_SERVICE_DOCUMENT',
    //     p_module: 'IFINPROC',
    //     p_child: this.param,
        ...
        for (let i = 0; i < this.photolist.length; i++) {
            if (this.photolist[i].selected) {
                this.checkedList.push({
                    p_id: this.photolist[i].id,
                    p_paths: this.photolist[i].file_path,
                    p_file_name: this.photolist[i].file_name
                });
            }
        }
