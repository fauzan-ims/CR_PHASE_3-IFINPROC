...
@Component({
  selector: 'app-root',
  templateUrl: './verificationapproval.component.html'
})
export class VerificationApprovalComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  private APIControllerItem: String = 'ProcurementRequestItem'
  ...
  loadData() {
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        // end param tambahan untuk getrows dynamic
        this.dalservice.Getrows(dtParameters, this.APIControllerItem, this.APIRouteForGetRows).subscribe(resp => {
          const parse = JSON.parse(resp);
