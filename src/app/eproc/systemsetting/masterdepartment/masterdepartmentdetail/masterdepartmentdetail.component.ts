...
@Component({
  selector: 'app-root',
  templateUrl: './masterdepartmentdetail.component.html'
})
export class MasterdepartmentdetailComponent extends BaseComponent implements OnInit {
  // get param from url
  ...
  private RoleAccessCode = 'R00011070000000A'; // role access 
  ...
  //#region form submit
  onFormSubmit(masterdepartmentForm: NgForm, isValid: boolean) {
    ...
    this.masterdepartmentData = masterdepartmentForm;
        ...
        .subscribe(
          res => {
            ...
            if (parse.result === 1) {
              this.showSpinner = false;
              this.showNotification('bottom', 'right', 'success');
              this.route.navigate(['/systemsetting/submasterdepartment/masterdepartmentdetail', parse.code]);
            } else {
              this.showSpinner = false;
              this.swalPopUpMsg(parse.data)
            }
  ...
  //#region button back
  btnBack() {
     this.route.navigate(['/systemsetting/submasterdepartment']);
