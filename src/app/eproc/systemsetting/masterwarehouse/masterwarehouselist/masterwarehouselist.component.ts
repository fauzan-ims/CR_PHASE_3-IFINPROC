...
@Component({
  moduleId: module.id,
  selector: 'app-root',
  templateUrl: './masterwarehouselist.component.html',
})

export class MasterWarehouselistComponent extends BaseComponent implements OnInit {
  // get param from url

  // variable
  public listmasterwarehouse: any = [];
    ...
    this.dtOptions = {
      ...
      ajax: (dtParameters: any, callback) => {
        ...
        this.dalservice.Getrows(dtParameters, this.APIController, this.APIRouteForGetRows).subscribe(resp => {
          ...
          this.listmasterwarehouse = parse.data;
          if (parse.data != null) {
            this.listmasterwarehouse.numberIndex = dtParameters.start;
          }
  ...
  //#region button add
  btnAdd() {
    this.route.navigate(['/systemsetting/submasterwarehouselist/masterwarehousedetail']);
  }
  //#endregion button add

  //#region button edit
  btnEdit(codeEdit: string) {
    this.route.navigate(['/systemsetting/submasterwarehouselist/masterwarehousedetail', codeEdit]);
  }
  ...
  btnDeleteAll() {
    this.checkedList = [];
    for (let i = 0; i < this.listmasterwarehouse.length; i++) {
      if (this.listmasterwarehouse[i].selected) {
        this.checkedList.push({ 'code': this.listmasterwarehouse[i].code, 'company_code': this.listmasterwarehouse[i].company_code });
      }
    }
  ...
  selectAllTable() {
    for (let i = 0; i < this.listmasterwarehouse.length; i++) {
      this.listmasterwarehouse[i].selected = this.selectedAllTable;
    }
  }

  checkIfAllTableSelected() {
    this.selectedAllTable = this.listmasterwarehouse.every(function (item: any) {
